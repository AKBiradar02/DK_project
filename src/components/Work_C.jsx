...
const SomeOfOurWork = () => {
  ...
  const [centerIndex, setCenterIndex] = useState(1); // middle one by default
  const [animate, setAnimate] = useState(false);

  const images = projectImages[currentBatch];

  // Compute reordered batch based on centerIndex
  const reordered = [
    images[(centerIndex + 2) % 3], // left
    images[centerIndex],          // center
    images[(centerIndex + 1) % 3] // right
  ];

  // Animate on center change
  useEffect(() => {
    setAnimate(true);
  ...
  const handleRecenter = (indexInView) => {
    if (indexInView === 0) setCenterIndex((centerIndex + 2) % 3); // clicked left
    if (indexInView === 2) setCenterIndex((centerIndex + 1) % 3); // clicked right
  };
  ...
  return (
    <>
      ...
      <section className="bg-white pt-18 pb-16 px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative flex flex-col md:flex-row justify-center pt-20 md:pt-0 px-4 sm:px-6 lg:px-8 mx-auto max-w-screen-xl items-center h-auto md:h-96 overflow-visible md:overflow-visible">
          ...
          {projectImages.map((_, index) => (
            <button
              ...
              onClick={() => {
                ...
                setCenterIndex(1); // reset center on new batch
              }}
